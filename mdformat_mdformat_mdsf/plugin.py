...
from .mdit_plugins import mdformat_mdsf_plugin


def add_cli_argument_group(group: argparse._ArgumentGroup) -> None:
    """Add options to the mdformat CLI.

    Stored in `mdit.options["mdformat"]["plugin"]["mdformat_mdsf"]`
...
def update_mdit(mdit: MarkdownIt) -> None:
    """Update the parser."""
    mdit.use(mdformat_mdsf_plugin)


def _render_mdformat_mdsf(node: RenderTreeNode, context: RenderContext) -> str:
...
RENDERERS: Mapping[str, Render] = {
    "<placeholder>": _render_mdformat_mdsf,
}
